   ...
   *)
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec filt func xs ys =  // write a function which filters based on the specified criteria
                match xs with
                | [] -> ys
                | x::rest ->
                    match func x with
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            let rec filt xs ys = // write a function to filter for odd elements only.
                match xs with
                | [] -> ys
                | x::rest -> 
                    match (x%2 = 0) with
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            let rec folder initialState xs element = // write a function to do what's described above
                match xs with
                | [] -> element
                | x::rest -> folder element rest (initialState+element)
            folder initialState xs 0
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec folder initialState xs element = // write a function to multiply the elements of a list
                match xs with
                | [] -> element 
                | x::rest -> folder element rest (element*initialState)
            folder initialState xs 1
    ...
    let ``18 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
            let rec exist func xs = // Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
                match xs with
                | [] -> false
                | x::rest -> 
                    match func x with 
                    | true -> true
                    | false -> exist func rest
            exist f xs
    ...
    let ``21 tryFind: find the first matching element, if any`` () =
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec finder func xs = // Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
                match xs with
                | [] -> None
                | x::rest ->
                    match func x with
                    | true -> (Some x)
                    | false -> finder func rest
            finder p xs
    ...
    let ``22 tryPick: find the first matching element, if any, and transform it`` () =
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
            let rec picker func xs = // Does this: https://msdn.microsoft.com/en-us/library/ee353814.aspx
                match xs with
                | [] -> None
                | x::rest -> 
                    match func x with
                    | None -> picker func rest
                    | _ -> func x
            picker p []
        let f x =
            match x<=45 with
    ...
    *)
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec choose func xs ys = // Does this: https://msdn.microsoft.com/en-us/library/ee353456.aspx
                match xs with
                | [] -> ys
                | x::rest -> 
                    match func x with
